...
%             for j = 1:size(Z,2)
%                 temps(1:2)=Z(1:2,j);
%                 validate = (temps-x)'*inv(P)*(temps-x);
%                 if validate < gSquared
%                     gated(j) = true; %% BRUK NIS %% 
%                 end
%             end
        end
        ...
        function ll = loglikelihoodRatios(obj, Z, x, P)
            ...
            for j = 1:m
                temps(1:2) = Z(1:2,j);
                llCond(j) = EKF.loglikelihood(temps, x, P);
        ...
        function beta = associationProbabilities(obj, Z, x, P)
           ...
           % probabilities
           beta = exp(lls-logSumExp(lls)); % Er dette riktig?
        end
...
%             for j = 1:m 
%                 temps(1:2) = Z(1:2,j);
%                [xupd(:, j + 1), Pupd(:, :, j + 1)] = EKF.update(temps,x,P);
%             end
        end
